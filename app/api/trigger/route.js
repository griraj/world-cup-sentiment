...
// Called by the browser to trigger a data refresh.
// Runs the ingest pipeline directly (no internal HTTP call needed).
...
async function updateMetrics(db, posts) {
  ...
  const { data: existing } = await db
    .from('metrics_minutely').select('*')
    .eq('bucket_time', bucket).is('team_tag', null).maybeSingle()
  ...
  if (existing) {
    ...
    await db.from('metrics_minutely').insert({
      bucket_time: bucket, team_tag: null,
      post_count: n, positive_count: pos,
